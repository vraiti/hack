...
# Builds a venv from a profile's `venv` map spec (see run-remote.sh): a JSON
# object of {content: type}, type one of "python"/"package"/"requirements"/
# "script", insertion order preserved. Invoked with the venv's already
# hash-addressed destination dir, the spec JSON (as a file, so odd
# characters in script content never have to survive a shell arg), and the
# synced project root (cwd for "requirements"/"script" entries, so relative
# paths and project-relative commands resolve).
...
python_count=$(jq -r '[to_entries[] | select(.value == "python")] | length' "$SPEC_FILE")
if [[ "$python_count" -gt 1 ]]; then
    echo "ERROR: venv spec has more than one \"python\" entry" >&2
    exit 1
fi
PYTHON_VERSION="$(jq -r '[to_entries[] | select(.value == "python") | .key][0] // empty' "$SPEC_FILE")"
...
# Everything but "python" is applied in the spec's own key order -- installs
# and setup scripts can be order-dependent (e.g. a script that assumes an
# earlier package is already installed), which is also why this venv is
# hashed on the spec's exact JSON rather than an order-independent digest.
...
done < <(jq -c 'to_entries[] | [.value, .key]' "$SPEC_FILE")
